...
def test_branch_behind(zenith_env_builder: ZenithEnvBuilder):
    ...
    test_branch_behind_timeline_id = env.zenith_cli.create_timeline()
    pgmain = env.postgres.create_start('test_branch_behind',
                                       timeline=test_branch_behind_timeline_id)
    ...
    # Branch at the point where only 100 rows were inserted
    test_branch_behind_hundred_timeline_id = env.zenith_cli.create_timeline(
    ...
    test_branch_behind_more_timeline_id = env.zenith_cli.create_timeline(
        ancestor_timeline_id=test_branch_behind_timeline_id, ancestor_start_lsn=lsn_b)

    pg_hundred = env.postgres.create_start("test_branch_behind_hundred",
                                           timeline=test_branch_behind_hundred_timeline_id)
    pg_more = env.postgres.create_start("test_branch_behind_more",
                                        timeline=test_branch_behind_more_timeline_id)
    ...
    test_branch_segment_boundary_timeline_id = env.zenith_cli.create_timeline(
        ancestor_timeline_id=test_branch_behind_timeline_id, ancestor_start_lsn="0/3000000")
    pg = env.postgres.create_start("test_branch_segment_boundary",
                                   timeline=test_branch_segment_boundary_timeline_id)
